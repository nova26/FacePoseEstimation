...
def wrtieInputFileForFPNModel():
    ...
    imagesToCopy = glob.glob("C:\\Users\\Shahar\\Desktop\\PythonProjects\\ComputerVision\\Project\\profile_images/*.jpg")
...
def trainFrontalCNN():
    ...
    IMAGE_SIZE = 250
    MAX_IMAGES_TO_LOAD = 10000000

    frontalImagesToTest = glob.glob("./data\\HELEN/*.jpg")
    frontalImagesResults = glob.glob("./data\\FPN_HELLEN_RESULTS/*.txt")

    imageNameTo3DOFGround = {}

    if os.path.isfile('./FrontalFacePoseDetectorCNN_pickles/imageNameTo3DOFGround.pkl'):
        imageNameTo3DOFGround = pickle.load(open("./FrontalFacePoseDetectorCNN_pickles/imageNameTo3DOFGround.pkl", "rb"))
    else:
        count =1
        size = len(frontalImagesResults)
        for file in frontalImagesResults:
        ...
        pickle.dump(imageNameTo3DOFGround, open("./FrontalFacePoseDetectorCNN_pickles/imageNameTo3DOFGround.pkl", "wb"))



    if not os.path.isfile('./FrontalFacePoseDetectorCNN_pickles/ImageSet/profileImages_200.pkl'):

        profileImages = np.full((1, IMAGE_SIZE, IMAGE_SIZE, 3), 0)
        profileDOFlabels = np.full((1, 3), 0)
        x = 1
        s = len(frontalImagesToTest)

        numberOfImagesToLoad = 10001

        groundImagesNames = imageNameTo3DOFGround.keys()

        for im in frontalImagesToTest:
            n = im.replace(".jpg", "")
            n = n[n.rfind('\\') + 1:]
            ...
            image = cv2.copyMakeBorder(image.copy(), heightPaddintTop, heightPaddintButton, widthPaddingRight, widthPaddingLeft, cv2.BORDER_CONSTANT, value=[0,0,0])
            ...
            if x % 200 == 0:
                ...
                pickle.dump(profileImages, open("./FrontalFacePoseDetectorCNN_pickles/ImageSet/profileImages_{0}.pkl".format(x), "wb"))
                pickle.dump(profileDOFlabels, open("./FrontalFacePoseDetectorCNN_pickles/ImageSet/profileDOFlabels_{0}.pkl".format(x), "wb"))
    ...
    print(200)
    profileImages = pickle.load(open("./FrontalFacePoseDetectorCNN_pickles/ImageSet/profileImages_{0}.pkl".format(200), "rb"))
    profileDOFlabels = pickle.load(open("./FrontalFacePoseDetectorCNN_pickles/ImageSet/profileDOFlabels_{0}.pkl".format(200), "rb"))

    for x in range(400, 10200, 200):
        profileImagesTmp = pickle.load(open("./FrontalFacePoseDetectorCNN_pickles/ImageSet/profileImages_{0}.pkl".format(x), "rb"))
        profileDOFlabelsTmp = pickle.load(open("./FrontalFacePoseDetectorCNN_pickles/ImageSet/profileDOFlabels_{0}.pkl".format(x), "rb"))
...
#  profileImages = np.full((1, IMAGE_SIZE, IMAGE_SIZE, 3), 0)
    ...
    modelCheckPoints = keras.callbacks.ModelCheckpoint("./FrontalFacePoseDetectorCNN_pickles/Model/"+fileName,
                                                                 monitor='val_loss',
                                                                 verbose=1,
                                                                 save_best_only=True,
    ...
    pickle.dump(history.history, open("./FrontalFacePoseDetectorCNN_pickles/profileImagesModel_history.pkl", "wb"))
